        ...
        <div class="ao-today-intro">
          ...
          <div class="ao-intro-actions">
            <Show when={nextTodo()} fallback={<button type="button" class="ao-aqua-button" onClick={props.onOpenTodos}><Plus size={16} aria-hidden="true" /> START / 开始专注</button>}>
              {(item) => <button type="button" class="ao-aqua-button" disabled={props.busy() || props.timerHasProgress()} onClick={() => props.onStartTodo(item())}><Play size={16} fill="currentColor" aria-hidden="true" /> START / 开始专注 <ArrowRight size={15} aria-hidden="true" /></button>}
            </Show>
        ...
        <div class="ao-orbit-stage" aria-label="今日潮汐轨道">
          ...
          <For each={sequence()}>
            {(item, index) => {
              ...
              return (
                <button type="button" classList={{ "ao-orbit-node": true, "is-complete": completed, "is-current": active }} style={"left:" + String(point.left) + "%;top:" + String(point.top) + "%;"} aria-label={`${item.title}，${item.scheduledTime || (completed ? "已完成" : "待安排")}`} title={item.title} disabled={props.busy() || completed || props.timerHasProgress()} onClick={() => props.onStartTodo(item)}>
      ...
      <div class="ao-today-rail">
        <AoGlass class="ao-next-capsule">
          ...
          <Show when={nextTodo()} fallback={<><strong>等待一个念头浮上来</strong><p>去待办里写下下一步，轨道会为它留出位置。</p><button type="button" class="ao-text-button" onClick={props.onOpenTodos}>添加待办 <ArrowRight size={14} /></button></>}>
            {(item) => <><strong>{item().title}</strong><p>{props.formatTodoDue(item())} · 建议 {props.defaultFocusMinutes()} 分钟</p><button type="button" class="ao-text-button" disabled={props.busy() || props.timerHasProgress()} onClick={() => props.onStartTodo(item())}>让它成为焦点 <ArrowRight size={14} /></button></>}
          </Show>
        </AoGlass>
...
export function AuroraOceanFocus(props: FocusSurfaceProps) {
  ...
  const recentRecords = createMemo(() => props.records().slice(0, 4));
    ...
    <section class="ao-page ao-focus-page" aria-label="潮汐计时">
      ...
      <div class="ao-focus-hero">
        <div class="ao-focus-copy">
          ...
          <div class="ao-wave-history"><span class="ao-section-label">RECENT WAVES / 最近片段</span><For each={recentRecords()}>{(item, index) => <div class="ao-wave-history__row"><b>{String(index() + 1).padStart(2, "0")}</b><span>{item.title}</span><small>{item.completedTime || "未填写时间"} · {item.durationLabel}</small><i /></div>}</For><Show when={recentRecords().length === 0}><p class="ao-muted">完成第一段专注后，潮汐会在这里留下痕迹。</p></Show><button type="button" class="ao-ghost-link" onClick={props.onOpenRecords}>打开记录 <ChevronRight size={14} /></button></div>
        </div>
...
export function AuroraOceanTodos(props: TodoSurfaceProps) {
  ...
  return (
    <section class="ao-page ao-todos-page" aria-label="潮汐清单">
      ...
      <div class="ao-reef-board">
        <section class="ao-reef-zone ao-reef-zone--now"><header><span class="ao-zone-mark">01</span><div><span>BY DATE / 按日期</span><strong>{pendingTodoCount()} 个气泡</strong></div><AoPulse /></header><div class="ao-reef-list ao-reef-list--dates"><Show when={props.ready()} fallback={<p class="ao-muted">正在读取潮汐…</p>}><Show when={pendingDateGroups().length > 0} fallback={<div class="ao-reef-empty"><span>○</span><strong>这里很安静</strong><small>把正在靠近的事项放进来。</small></div>}><TodoDateGroupList groups={pendingDateGroups} listLabel="按日期分组的待办" renderItem={(item) => <AuroraTodoCard item={item} {...rowProps} />} editingTodo={props.editingTodo} busy={props.busy} timerHasProgress={props.timerHasProgress} formatTodoDue={props.formatTodoDue} importanceLabel={props.importanceLabel} onToggle={props.onToggle} onBeginEdit={props.onBeginEdit} onUseForFocus={props.onUseForFocus} onRemove={props.onRemove} onPatch={props.onPatch} onSave={props.onSave} onCancel={props.onCancel} /></Show></Show></div></section>
        <section class="ao-reef-zone ao-reef-zone--arrived"><header><span class="ao-zone-mark">02</span><div><span>ARRIVED / 已抵达</span><strong>{props.completedTodos().length} 个气泡</strong></div><span class="ao-checkmark"><Check size={13} /></span></header><div class="ao-reef-list ao-reef-list--completed"><For each={props.completedTodos()}>{(item) => <AuroraTodoCard item={item} {...rowProps} />}</For><Show when={props.completedTodos().length === 0}><div class="ao-reef-empty"><span>✦</span><strong>完成一件，它会抵达这里</strong><small>完成不是终点，是今天留下的光。</small></div></Show></div></section>
      </div>
...
export function AuroraOceanRecords(props: RecordsSurfaceProps) {
  ...
  return (
    <section class="ao-page ao-records-page" aria-label="潮汐记录">
      <header class="ao-page-top"><div><span class="ao-eyebrow">TIDE ARCHIVE / {props.formatAnalyticsDate(selectedDate())}</span><span class="ao-breadcrumb">深海光场 · 记录回看</span></div><div class="ao-records-head"><span><i class="ao-status-dot" /> LOCAL ARCHIVE</span><button type="button" class="ao-aqua-button" disabled={props.busy()} onClick={() => void props.onCreateManualRecord?.()}>补录专注</button></div></header>
      ...
      <AoGlass title="SEVEN DAY TIDE / 七日潮汐" note={props.formatAnalyticsDate(selectedDate())} class="ao-archive-chart-card"><div class="ao-archive-chart"><div class="ao-archive-axis"><span>高</span><span>中</span><span>低</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="七日专注潮汐图"><path class="ao-archive-grid" d="M0 28H100 M0 49H100 M0 70H100" /><path class="ao-archive-path" d={props.archivePath()} /></svg><div class="ao-archive-points" aria-label="七日专注日期"><For each={props.archiveDays()}>{(day, index) => { const intensity = day.totalDurationMs / maxDuration(); const x = props.archiveDays().length <= 1 ? 50 : 7 + (86 * index()) / (props.archiveDays().length - 1); const y = 70 - intensity * 42; return <button type="button" classList={{ "ao-archive-point": true, active: day.date === selectedDate() }} style={`left:${x}%;top:${y}%;`} onClick={() => props.onSelectDate(day.date)}><i /><span>{props.formatAnalyticsDate(day.date)}</span></button>; }}</For></div></div><div class="ao-chart-legend"><span><i class="ao-status-dot" /> 每个光点代表一天</span><strong>最近七天累计 {props.formatDurationMs(props.recentWeekDurationMs())}</strong><span>{props.recentWeekActiveDays()} 天有投入</span></div></AoGlass>
      <div class="ao-record-stat-grid"><div><span>总投入</span><strong>{aoAnalytics(props.analytics(), "totalFocusDurationLabel", "00:00:00")}</strong><small>全部潮汐</small></div><div><span>完成段数</span><strong>{aoAnalytics(props.analytics(), "sessionCount", "0")}</strong><small>光点</small></div><div><span>平均潮汐</span><strong>{props.analytics()?.sessionCount ? props.formatDurationMs(props.recentWeekDurationMs() / Math.max(1, props.analytics()?.sessionCount ?? 1)) : "00:00:00"}</strong><small>每一段</small></div><div><span>节奏成就</span><strong>{aoAnalytics(props.analytics(), "currentStreakDays", "0")}</strong><small>下一次回来从今天算起</small></div></div>
      <div class="ao-records-lower">
        <AoGlass title="WAVE LOG / 当日潮汐" note={String(selectedRecords().length) + " EVENTS"} class="ao-wave-log"><div class="ao-wave-log__head"><span>时间</span><span>专注意图</span><span>时长</span><span>动作</span></div><Show when={visibleRecords().length > 0} fallback={<div class="ao-empty-records">这一天还没有潮汐。选择另一颗光点，或现在开始一段。</div>}><For each={visibleRecords()}>{(record) => <Show when={props.editingRecord()?.id === record.id} fallback={<div class="ao-wave-row"><time>{aoTime(record)}</time><div><strong title={record.title}>{record.title}</strong><small>{props.formatRecordDate(record)} · {record.source === "manual" ? "手动补录" : "计时完成"}{record.editedAt ? " · 已修正" : ""}</small></div><b>{record.durationLabel}</b><div class="ao-wave-row__actions"><button type="button" class="ao-card-action" onClick={() => props.onBeginEdit(record)}>改名</button><Show when={props.onBeginDetailedEdit}><button type="button" class="ao-card-action" onClick={() => props.onBeginDetailedEdit?.(record)}>详细编辑</button></Show><button type="button" class="ao-card-action ao-card-action--danger" onClick={() => void props.onRemove(record.id)}>移除</button></div></div>}><div class="ao-wave-row ao-wave-row--editing"><input aria-label="编辑记录标题" value={props.editingRecord()?.title ?? record.title} onInput={(event) => props.onPatchEdit(event.currentTarget.value)} /><button type="button" class="ao-text-button" onClick={() => void props.onSaveEdit()}>保存</button><button type="button" class="ao-ghost-link" onClick={props.onCancelEdit}>取消</button></div></Show>}</For><Show when={visibleCount() < selectedRecords().length}><button type="button" class="ao-ghost-link" onClick={() => setVisibleCount((count) => Math.min(count + 12, selectedRecords().length))}>加载更多 · {visibleCount()} / {selectedRecords().length}</button></Show></Show></AoGlass>
...
export function AuroraOceanSettings(props: SettingsSurfaceProps) {
  ...
  return (
    <section class="ao-page ao-settings-page" aria-label="光场设置">
      ...
      <aside class="ao-settings-preview"><div class="ao-preview-orbit"><div class="ao-preview-orbit__core"><span>LIVE</span><strong>{activeTheme().name}</strong></div><i /><i /><i /></div><span>ACTIVE LIGHT FIELD</span><strong>{activeTheme().englishName}</strong><p>{activeTheme().description}</p><small class="ao-settings-preview__status">主题、亮度、动效与密度都会即时保存。</small></aside>
      </div>
    </section>
  );
}
