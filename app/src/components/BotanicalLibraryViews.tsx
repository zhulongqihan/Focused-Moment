...
export function BotanicalLibraryRecords(props: RecordsSurfaceProps) {
  ...
  return (
    <section class="bl-page bl-records-page" aria-label="成长记录">
      ...
      <BlPaper title="THE READING GARDEN / 七日生长" note={props.formatAnalyticsDate(selectedDate())} class="bl-growth-chart-card"><div class="bl-growth-chart"><div class="bl-growth-shelf-line" /><div class="bl-growth-axis"><span>长得高</span><span>扎根</span><span>播种</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="七日专注生长图"><path class="bl-growth-grid" d="M0 28H100 M0 49H100 M0 70H100" /><path class="bl-growth-path" d={props.archivePath()} /></svg><div class="bl-growth-points" aria-label="七日专注日期"><For each={props.archiveDays()}>{(day, index) => { const intensity = day.totalDurationMs / maxDuration(); const x = props.archiveDays().length <= 1 ? 50 : 7 + (86 * index()) / (props.archiveDays().length - 1); const y = 70 - intensity * 42; return <button type="button" classList={{ "bl-growth-point": true, active: day.date === selectedDate() }} style={`left:${x}%;top:${y}%;`} onClick={() => props.onSelectDate(day.date)}><i /><span>{props.formatAnalyticsDate(day.date)}</span></button>; }}</For></div></div><div class="bl-chart-legend"><span><i class="bl-status-dot" /> 每一株代表一天</span><strong>最近七日 {props.formatDurationMs(props.recentWeekDurationMs())}</strong><span>{props.recentWeekActiveDays()} 天有投入</span></div></BlPaper>
...
export function BotanicalLibrarySettings(props: SettingsSurfaceProps) {
  ...
  return (
    <section class="bl-page bl-settings-page" aria-label="书房设置">
      ...
      <div class="bl-settings-intro"><span class="bl-kicker">LIBRARY RULES / 书房设置</span><h1>整理一间<br /><em>你愿意久坐的房间。</em></h1><p>主题、提醒和本地备份都放在书架旁。调整后会立即应用并自动保存。</p></div>
      ...
      </div><aside class="bl-settings-preview"><div class="bl-preview-room"><div class="bl-preview-lamp" /><div class="bl-preview-shelf"><i /><i /><i /><i /></div><div class="bl-preview-paper"><span>LIVE CATALOG</span><strong>{activeTheme().name}</strong></div></div><span>ACTIVE READING ROOM</span><strong>{activeTheme().englishName}</strong><p>{activeTheme().description}</p><small class="bl-settings-preview__status">主题、亮度、动效与密度都会即时保存。</small></aside></div>
    </section>
  );
}
