...
export function BotanicalLibraryToday(props: TodaySurfaceProps) {
  ...
  return (
    <section class="bl-page bl-today-page" aria-label="今日生长">
      <header class="bl-library-head"><div><span class="bl-eyebrow">BOTANICAL LIBRARY / {blDate(props.todayDate)}</span><span class="bl-breadcrumb">木质书房 · 今日生长</span></div><div class="bl-reading-chip"><i class="bl-status-dot" /> 今日完成 <strong>{String(props.todayCompletedTodos().length).padStart(2, "0")}</strong> 项</div></header>
      <div class="bl-today-intro"><div><span class="bl-kicker">GROWTH / {props.todayLabel}</span><h2 class="bl-surface-title">GROWTH / 今日生长</h2><h1>让今天<br /><em>长出一圈年轮。</em></h1><p>把注意力放在书桌上，一件一件地照看。完成不是清空，而是留下新的生长线。</p><DailyFocusLine date={props.todayDate} theme="botanical-library" /><div class="bl-intro-actions"><Show when={nextTodo()} fallback={<button type="button" class="bl-ink-button" onClick={props.onOpenTodos}><Plus size={16} /> 先添加一件待办</button>}>{(item) => <button type="button" class="bl-ink-button" disabled={props.busy() || props.timerHasProgress()} onClick={() => props.onStartTodo(item())}><Play size={16} fill="currentColor" /> START / 开始专注 <ArrowRight size={15} /></button>}</Show><button type="button" class="bl-text-link" onClick={props.onOpenRecords}>翻开成长记录 <ChevronRight size={15} /></button></div></div><div class="bl-library-stilllife"><div class="bl-stilllife-lamp" /><div class="bl-stilllife-shelf"><For each={sequence()}>{(item, index) => <button type="button" classList={{ "bl-plant-marker": true, "is-complete": item.isCompleted, "is-current": index() === props.todayCompletedTodos().length }} disabled={props.busy() || item.isCompleted || props.timerHasProgress()} onClick={() => props.onStartTodo(item)}><span class="bl-pot" /><i class="bl-stem" /><b>{String(index() + 1).padStart(2, "0")}</b><strong>{item.title}</strong><small>{item.scheduledTime || (item.isCompleted ? "已完成" : "待安排")}</small></button>}</For><Show when={sequence().length === 0}><div class="bl-shelf-empty"><CircleDot size={24} /><strong>书架还没有新的生长点</strong><span>添加一件待办，开始今天的第一圈。</span></div></Show></div><div class="bl-stilllife-paper"><span>NEXT NOTE</span><Show when={nextTodo()} fallback={<><strong>留一页空白</strong><small>去待办里写下下一件要照看的事。</small></>}>{(item) => <><strong>{item().title}</strong><small>{props.formatTodoDue(item())}</small></>}</Show></div></div></div>
      <div class="bl-today-ledger"><div><span>今日完成</span><strong>{String(props.todayCompletedTodos().length).padStart(2, "0")}</strong><small>项</small></div><div><span>今日专注</span><strong>{focusDuration()}</strong><small>时长</small></div><div><span>书房状态</span><strong>{blTimerLabel(props.timer(), props.timerHasProgress())}</strong><small>{props.timer().mode}</small></div><div><span>待办队列</span><strong>{String(props.todayTodos().length).padStart(2, "0")}</strong><small>待处理</small></div></div><BlStatusLine analytics={props.analytics} timer={props.timer} />
    </section>
  );
}
...
export function BotanicalLibraryFocus(props: FocusSurfaceProps) {
  ...
  const recentItems = createMemo(() => props.records().slice(0, 4));
  ...
  return (
    <section class="bl-page bl-focus-page" aria-label="年轮计时">
      ...
      <div class="bl-focus-hero">
        <div class="bl-focus-notes"><span class="bl-kicker">FOCUS / 年轮计时</span><h1>在这一页上<br /><em>慢慢长大。</em></h1><p>把时间摊开，像一本值得反复阅读的书。下一行从这里开始。</p><div class="bl-bookmark-list"><span class="bl-section-label">RECENT RECORDS / 最近记录</span><For each={recentItems()}>{(item, index) => <div><b>{String(index() + 1).padStart(2, "0")}</b><span>{item.title}</span><small>{item.completedTime || "未填写时间"} · {item.durationLabel}</small></div>}</For><Show when={recentItems().length === 0}><p class="bl-muted">完成一段专注后，记录会留在这里。</p></Show><button type="button" class="bl-text-link" onClick={props.onOpenRecords}>查看记录 <ChevronRight size={14} /></button></div></div>
  ...
  return (
    <section class="bl-page bl-todos-page" aria-label="书房待办">
      ...
      <div class="bl-desk-board">
        <section class="bl-desk-column bl-desk-column--seed"><header><span>01</span><div><strong>BY DATE / 按日期</strong><small>{pendingTodoCount()} 件 · 现在照看</small></div><i /></header><div class="bl-task-list bl-task-list--dates"><Show when={props.ready()} fallback={<p class="bl-muted">正在读取书架…</p>}><Show when={pendingDateGroups().length > 0} fallback={<div class="bl-column-empty"><span>✿</span><strong>土壤是空的</strong><small>放一张纸条进来。</small></div>}><TodoDateGroupList groups={pendingDateGroups} listLabel="按日期分组的待办" renderItem={(item) => <BotanicalTodoCard item={item} {...rowProps} />} editingTodo={props.editingTodo} busy={props.busy} timerHasProgress={props.timerHasProgress} formatTodoDue={props.formatTodoDue} importanceLabel={props.importanceLabel} onToggle={props.onToggle} onBeginEdit={props.onBeginEdit} onUseForFocus={props.onUseForFocus} onRemove={props.onRemove} onPatch={props.onPatch} onSave={props.onSave} onCancel={props.onCancel} /></Show></Show></div></section>
        <section class="bl-desk-column bl-desk-column--harvest"><header><span>02</span><div><strong>HARVEST / 收成</strong><small>{props.completedTodos().length} 件 · 已留下年轮</small></div><span class="bl-harvest-mark"><Check size={13} /></span></header><div class="bl-task-list bl-task-list--completed"><For each={props.completedTodos()}>{(item) => <BotanicalTodoCard item={item} {...rowProps} />}</For><Show when={props.completedTodos().length === 0}><div class="bl-column-empty"><span>◌</span><strong>完成一件，它会在这里结果</strong><small>每一次收成都会留下一圈年轮。</small></div></Show></div></section>
      </div>
...
export function BotanicalLibraryRecords(props: RecordsSurfaceProps) {
  ...
  return (
    <section class="bl-page bl-records-page" aria-label="成长记录">
      <header class="bl-library-head"><div><span class="bl-eyebrow">GROWTH ARCHIVE / {props.formatAnalyticsDate(selectedDate())}</span><span class="bl-breadcrumb">木质书房 · 植物档案</span></div><div class="bl-records-head"><span><i class="bl-status-dot" /> LOCAL SHELF</span><button type="button" class="bl-ink-button" disabled={props.busy()} onClick={() => void props.onCreateManualRecord?.()}>补录专注</button></div></header>
...
export function BotanicalLibrarySettings(props: SettingsSurfaceProps) {
  ...
  return (
    <section class="bl-page bl-settings-page" aria-label="书房设置">
      ...
      <div class="bl-settings-intro"><span class="bl-kicker">LIBRARY RULES / 书房设置</span><h1>整理一间<br /><em>你愿意久坐的房间。</em></h1><p>主题、提醒和本地备份都放在书架旁，调整会立即生效。</p></div>
      <div class="bl-settings-grid"><nav class="bl-settings-nav" aria-label="设置分组"><a href="#bl-appearance">01 <span>书架</span></a><a href="#bl-behavior">02 <span>提醒</span></a><a href="#bl-audio">03 <span>声音</span></a><a href="#bl-rhythm">04 <span>节奏</span></a><a href="#bl-data">05 <span>备份</span></a></nav><div class="bl-settings-main">
        <BlPaper title="THE BOOKSHELF / 主题书架" note="APPEARANCE / 01" class="bl-setting-card"><div id="bl-appearance" class="bl-theme-books"><ThemePicker value={props.themeId()} onChange={props.onThemeSelect} className="bl-theme-picker" /></div><div class="bl-setting-sliders"><label><span>灯光亮度 <b>{props.visualIntensity()}%</b></span><input type="range" min="0" max="100" value={props.visualIntensity()} aria-label="灯光亮度" onInput={(event) => props.onVisualIntensityChange(Number(event.currentTarget.value))} /></label><label><span>叶影动效 <b>{props.motionIntensity()}%</b></span><input type="range" min="0" max="100" value={props.motionIntensity()} aria-label="叶影动效" onInput={(event) => props.onMotionIntensityChange(Number(event.currentTarget.value))} /></label><label><span>桌面密度 <b>{props.density() === "roomy" ? "舒展" : "紧凑"}</b></span><div class="bl-density-buttons"><button type="button" classList={{ active: props.density() === "roomy" }} aria-pressed={props.density() === "roomy"} onClick={() => props.onDensityChange("roomy")}>舒展</button><button type="button" classList={{ active: props.density() === "compact" }} aria-pressed={props.density() === "compact"} onClick={() => props.onDensityChange("compact")}>紧凑</button></div></label></div><label class="bl-auto-mini-toggle"><input type="checkbox" checked={props.autoMiniOnStart()} onChange={(event) => props.onAutoMiniOnStartChange(event.currentTarget.checked)} /><span>开始专注时自动打开迷你工作台</span></label><Show when={props.appPreferenceSaveError()}><div class="bl-load-error" role="alert"><span>{props.appPreferenceSaveError()}</span><button type="button" class="bl-text-link" disabled={props.appPreferenceSaveBusy()} onClick={props.onRetryAppPreferenceSave}>重试保存</button></div></Show></BlPaper>
