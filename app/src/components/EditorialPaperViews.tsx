              ...
              <For each={props.todayTodos()}>
                {(item, index) => (
                  <button
                    ...
                    onClick={() => props.onStartTodo(item)}
                  >
        ...
        <aside class="ep-next-card" aria-label="下一件专注">
          ...
          >
            {(item) => <>
              ...
              <button type="button" class="ep-paper-button ep-paper-button--green" disabled={props.busy() || props.timerHasProgress()} onClick={() => props.onStartTodo(item())}>开始专注 <ArrowUpRight size={16} strokeWidth={1.6} aria-hidden="true" /></button>
            </>}
          </Show>
...
export function EditorialPaperFocus(props: FocusSurfaceProps) {
  ...
  return (
    <section class="ep-page ep-focus-page" aria-label="专注时刻">
      ...
      <footer class="ep-focus-footer"><span><Clock3 size={15} strokeWidth={1.6} aria-hidden="true" />{props.timerCanContinue() ? "可以继续回来" : "准备好时，从这里开始"}</span><Show when={props.timerHasProgress()}><button type="button" class="ep-secondary-button ep-focus-floating-link" title="打开迷你工作台" onClick={() => void props.onShowFocusFloating()}>打开迷你工作台 <ArrowUpRight size={15} strokeWidth={1.6} aria-hidden="true" /></button></Show><span>FOCUSED MOMENT · FM-2026</span><span>{props.timer().activeTaskTitle || "一段完整的注意力"}</span></footer>
    </section>
  );
}
  ...
  return (
    <section class="ep-page ep-records-page" aria-label="专注年鉴">
      <header class="ep-page-header ep-records-header">
        ...
        <h1>专注年鉴</h1>
        <p>回看节奏，给下一页留下证据。</p>
        <button type="button" class="ep-primary-button" disabled={props.busy()} onClick={() => void props.onCreateManualRecord?.()}>补录专注</button>
      </header>
      ...
      <section class="ep-full-history"><div class="ep-section-heading"><div><span class="ep-section-label">FULL INDEX / ALL RECORDS</span><h2>全部记录</h2></div><span>{props.records().length} 轮</span></div><Show when={props.ready() && props.records().length > 0} fallback={<p class="ep-empty">完成一次计时后，记录会显示在这里。</p>}><div class="ep-full-history__list" aria-label="按日期折叠的全部记录"><For each={props.recordGroups()}>{(group) => <details class="ep-history-day" open={expandedHistoryDate() === group.date}><summary aria-controls={`ep-history-${group.date}`} aria-expanded={expandedHistoryDate() === group.date} onClick={(event) => { event.preventDefault(); setExpandedHistoryDate((date) => date === group.date ? null : group.date); }}><span>{props.formatRecordDay(group.date)}</span><strong>{group.records.length} 轮 · {props.formatDurationMs(group.totalDurationMs)}</strong></summary><Show when={expandedHistoryDate() === group.date}><div id={`ep-history-${group.date}`}>{group.records.map((record) => <span>{record.title} · {record.durationLabel}</span>)}</div></Show></details>}</For></div></Show></section>
    </section>
  );
}
...
export function EditorialPaperSettings(props: SettingsSurfaceProps) {
  ...
  const motionMode = createMemo(() => {
    const value = props.motionIntensity();
    if (value <= 0) return { key: "off", label: "静息", detail: "停用装饰动效，只保留点击反馈。" };
    if (value < 50) return { key: "subtle", label: "轻盈", detail: "保留轻微呼吸与切换，让信息先于装饰。" };
    return { key: "full", label: "流动", detail: "启用完整的纸面过渡与微动效。" };
  });
  const densityMode = createMemo(() => props.density() === "roomy"
    ? { label: "舒展", detail: "卡片留白更大，适合逐项阅读。" }
    : { label: "紧凑", detail: "行距更短，一屏可以看到更多信息。" });
  ...
  return (
    <section class="ep-page ep-settings-page" aria-label="专注手册">
      ...
      <div class="ep-settings-spread">
        ...
        <section class="ep-settings-paper ep-settings-paper--workspace"><div class="ep-section-heading"><div><span class="ep-section-label">04 / WORKSPACE</span><h2>工作台布局</h2></div><SlidersHorizontal size={18} strokeWidth={1.4} aria-hidden="true" /></div><p class="ep-workspace-note">这里调整的是整套工作台的阅读节奏：明暗改变层次，动效改变切换的存在感，密度改变一屏能看到的信息量。</p><div class="ep-workspace-controls"><label class="ep-slider-row"><span>画面明暗 <b>{props.visualIntensity()}%</b></span><input type="range" min="0" max="100" value={props.visualIntensity()} aria-label="画面明暗" onInput={(event) => props.onVisualIntensityChange(Number(event.currentTarget.value))} /><small>只改变背景与纸面层次，不会隐藏内容。</small></label><label class="ep-slider-row"><span>动效程度 <b>{props.motionIntensity()}% · {motionMode().label}</b></span><input type="range" min="0" max="100" value={props.motionIntensity()} aria-label="动效程度" onInput={(event) => props.onMotionIntensityChange(Number(event.currentTarget.value))} /><small>{motionMode().detail}</small></label><div class="ep-density-row"><div><span>信息密度</span><small>{densityMode().detail}</small></div><div role="group" aria-label="信息密度"><button type="button" classList={{ selected: props.density() === "roomy" }} aria-pressed={props.density() === "roomy"} title="卡片留白更大，适合逐项阅读" onClick={() => props.onDensityChange("roomy")}>舒展</button><button type="button" classList={{ selected: props.density() === "compact" }} aria-pressed={props.density() === "compact"} title="行距更短，一屏看到更多信息" onClick={() => props.onDensityChange("compact")}>紧凑</button></div></div></div><div class="ep-workspace-preview" data-motion={motionMode().key} data-density={props.density()} style={`--ep-preview-opacity: ${0.55 + (props.visualIntensity() / 100) * 0.45}; --ep-preview-duration: ${Math.max(120, 900 - props.motionIntensity() * 7)}ms;`} aria-live="polite"><div class="ep-workspace-preview__heading"><span>LIVE WORKSPACE / 即时预览</span><strong>{densityMode().label} · {motionMode().label}</strong></div><div class="ep-workspace-preview__sheet"><div><span>今日专注</span><b>02:15:00</b></div><div><span>下一步</span><b>一件事 · 一个落点</b></div><i class="ep-workspace-preview__pulse" /></div><p>{densityMode().detail} {motionMode().detail}</p></div><p class="ep-workspace-apply-note">显示设置会跨主题保留，拖动或切换后立即生效。</p></section>
