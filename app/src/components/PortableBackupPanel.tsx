...
function selectedPath(event: Event) {
...
export default function PortableBackupPanel(props: PortableBackupPanelProps) {
  ...
  async function chooseBackupPath() {
    ...
    try {
      ...
      setDialogError(error instanceof Error ? error.message : "无法打开文件选择器。请手动填写路径后重试。");
    }
  }
  ...
  async function chooseExportPath() {
    ...
    try {
      ...
      setDialogError(error instanceof Error ? error.message : "无法打开保存位置选择器。请手动填写路径后重试。");
    }
  }

  return (
    <section class="portable-backup-panel" aria-label="便携备份">
      <header><div><span>PORTABLE BACKUP / V3</span><h2>完整备份与恢复</h2><p>预览不会修改当前数据；确认导入前会自动生成回滚备份。</p></div><div class="portable-backup-panel__file-actions"><button type="button" class="secondary-button" disabled={props.busy} onClick={() => void chooseBackupPath()}>选择 JSON 文件</button><button type="button" class="secondary-button" disabled={props.busy} onClick={() => void chooseExportPath()}>选择导出位置</button><label class="portable-backup-panel__file"><span>浏览器备用选择</span><input type="file" accept=".json,application/json" onChange={(event) => props.onPathChange(selectedPath(event))} /></label></div></header>
        <div class="portable-backup-panel__path"><input aria-label="外部备份文件路径" value={props.path} placeholder="外部备份文件路径，或选择 JSON 文件" onInput={(event) => props.onPathChange(event.currentTarget.value)} /><button type="button" class="secondary-button" disabled={props.busy || !props.path.trim()} onClick={() => void props.onPreview()}>预览</button><button type="button" class="secondary-button" disabled={props.busy || !props.path.trim()} onClick={() => void props.onExport()}>导出到此路径</button></div>
        <Show when={dialogError()}><p class="portable-backup-panel__error" role="alert">{dialogError()}</p></Show>
        <Show when={props.preview}>
          {(preview) => <div class="portable-backup-panel__preview"><div><span>应用版本</span><strong>{preview().appVersion}</strong></div><div><span>格式 / Schema</span><strong>v{preview().formatVersion} / v{preview().schemaVersion}</strong></div><div><span>导出时间</span><strong>{preview().exportedAt}</strong></div><div><span>数据</span><strong>{preview().todoCount} 项待办 · {preview().focusRecordCount} 条记录</strong></div><div><span>未完成计时</span><strong>{preview().hasRuntimeSession ? "包含" : "不包含"}</strong></div><div><span>外观与音效</span><strong>{preview().hasAppPreferences ? "包含设置" : "不包含设置"} · {preview().hasCustomAlertSound ? "含自定义音效" : "无自定义音效"}</strong></div><Show when={preview().warnings.length > 0}><div class="portable-backup-panel__warnings"><span>导入提示</span><ul><ForWarnings warnings={preview().warnings} /></ul></div></Show><label class="portable-backup-panel__restore-option"><input type="checkbox" checked={props.restoreAppPreferences} onChange={(event) => props.onRestoreAppPreferencesChange(event.currentTarget.checked)} /><span>同时恢复主题、视觉设置和自定义音效</span></label><div class="portable-backup-panel__actions"><button type="button" class="primary-button" disabled={props.busy} onClick={() => void props.onImport()}>确认导入并生成回滚</button></div></div>}
        </Show>
    </section>
  );
}
