...
param(
  ...
  [string]$Phase = "all",
  [string]$ExePath = (Join-Path $PSScriptRoot "..\src-tauri\target\debug\focused-moment.exe"),
  [string]$OutputDirectory = (Join-Path $PSScriptRoot "..\..\artifacts\qa\performance\PERF-01\f1060bb"),
  [ValidateRange(5, 60)]
...
$resolvedOutputDirectory = if ([System.IO.Path]::IsPathRooted($OutputDirectory)) {
  ...
  [System.IO.Path]::GetFullPath((Join-Path $appRoot $OutputDirectory))
}
