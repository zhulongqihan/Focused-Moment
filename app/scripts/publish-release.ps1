...
if (-not $NotesFile -and -not $Notes) {
  $defaultNotesFile = Join-Path $workspaceRoot ("docs\{0}\RELEASE_NOTES.md" -f $Tag)
...
if ($assets.Count -eq 0) {
  $versionedExe = Join-Path $releaseDir ("{0}\assets\Focused Moment {1}.exe" -f $Profile, $Tag)
  $versionedSetup = Join-Path $releaseDir ("{0}\assets\Focused Moment Setup {1}.exe" -f $Profile, $Tag)
  $versionedMsi = Join-Path $appRoot ("src-tauri\target\{0}\bundle\msi\Focused Moment_{1}_x64_en-US.msi" -f $Profile, $currentVersion)
