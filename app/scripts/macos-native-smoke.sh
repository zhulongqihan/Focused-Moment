...
app_path="${1:-$app_root/src-tauri/target/debug/bundle/macos/Focused Moment.app}"
app_executable="$app_path/Contents/MacOS/focused-moment"
smoke_root="${RUNNER_TEMP:-$workspace_root/artifacts/qa}/focused-moment-macos-native-${GITHUB_RUN_ID:-$$}"
...
dmg_path="$(find "$app_root/src-tauri/target/debug/bundle/dmg" -maxdepth 1 -type f -name '*.dmg' -print -quit)"
if [[ -z "$dmg_path" ]]; then
