...
## 10.1 维护后发布策略与 REL-15（2026-09-13）

维护阶段不再重复清理。本轮用户可见修复只限 Night Valley 计时页“查看专注记录”按钮的文字几何居中；现有箭头改为按钮右侧视觉提示，未触碰用户数据、备份、历史 Release 或旧归档。v2.10.12 仅作为 Windows-only 发布，macOS workflow 保留手动入口但不再随 push/tag 自动运行，直到用户明确授权恢复。
