...
export function BotanicalLibraryRecords(props: NightValleyRecordsProps) {
  ...
  return (
    <section class="bl-page bl-records-page" aria-label="成长记录">
      <header class="bl-library-head"><div><span class="bl-eyebrow">GROWTH ARCHIVE / {props.formatAnalyticsDate(selectedDate())}</span><span class="bl-breadcrumb">木质书房 · 植物档案</span></div><div class="bl-records-head"><span><i class="bl-status-dot" /> LOCAL SHELF</span><button type="button" class="bl-stone-button" disabled title="导出记录尚未接入"><Download size={14} /> 导出</button></div></header>
      <div class="bl-records-intro"><div><span class="bl-kicker">GROWTH ARCHIVE / 生长档案</span><h1>每一圈年轮<br /><em>都是读过的页。</em></h1></div><div class="bl-archive-range"><span>档案范围</span><strong>{props.formatArchiveRangeDate(props.archiveDays()[0]?.date ?? selectedDate())} — {props.formatArchiveRangeDate(props.archiveDays()[props.archiveDays().length - 1]?.date ?? selectedDate())}</strong><button type="button" disabled title="日期范围筛选尚未接入">⌄</button></div></div>
      <BlPaper title="THE READING GARDEN / 七日生长" note={props.formatAnalyticsDate(selectedDate())} class="bl-growth-chart-card"><div class="bl-growth-chart"><div class="bl-growth-shelf-line" /><div class="bl-growth-axis"><span>长得高</span><span>扎根</span><span>播种</span></div><svg viewBox="0 0 900 250" preserveAspectRatio="none" aria-label="七日专注生长图"><path class="bl-growth-grid" d="M0 48H900 M0 124H900 M0 202H900" /><path class="bl-growth-path" d={props.archivePath()} /><For each={props.archiveDays()}>{(day, index) => <button type="button" classList={{ "bl-growth-point": true, active: day.date === selectedDate() }} style={"left:" + String((index() / Math.max(1, props.archiveDays().length - 1)) * 100) + "%;bottom:" + String(Math.max(5, (day.totalDurationMs / maxDuration()) * 74)) + "%"} onClick={() => props.onSelectDate(day.date)}><i /><span>{props.formatAnalyticsDate(day.date)}</span></button>}</For></svg></div><div class="bl-chart-legend"><span><i class="bl-status-dot" /> 每一株代表一天</span><strong>最近七日 {props.formatDurationMs(props.recentWeekDurationMs())}</strong><span>{props.recentWeekActiveDays()} 天有投入</span></div></BlPaper>
  ...
  return (
    <section class="bl-page bl-settings-page" aria-label="书房设置">
      ...
      <div class="bl-settings-grid"><nav class="bl-settings-nav" aria-label="设置分组"><a href="#bl-appearance">01 <span>书架</span></a><a href="#bl-behavior">02 <span>提醒</span></a><a href="#bl-audio">03 <span>声音</span></a><a href="#bl-shortcuts">04 <span>快捷键</span></a><a href="#bl-data">05 <span>备份</span></a></nav><div class="bl-settings-main">
        ...
        <BlPaper title="BOOKMARKS / 快捷键" note="INPUT / 04" class="bl-setting-card"><div id="bl-shortcuts" class="bl-shortcut-list"><div><span>开始 / 继续当前专注</span><kbd>Ctrl</kbd><b>+</b><kbd>Enter</kbd></div><div><span>结束并保存这一段</span><kbd>Ctrl</kbd><b>+</b><kbd>Shift</kbd><b>+</b><kbd>E</kbd></div><div><span>全屏切换</span><kbd>F11</kbd></div></div></BlPaper>
