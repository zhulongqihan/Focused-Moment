...
export function NightValleyFocus(props: NightValleyFocusProps) {
  ...
  const routePath = "M 28 24 C 18 92, 88 132, 106 230 S 172 407, 206 413 S 340 440, 381 419 S 548 455, 613 447 S 785 430, 805 392 S 847 120, 845 24";
...
export interface NightValleySettingsProps {
  ...
  customAlertSoundInputRef: (element: HTMLInputElement) => void;
  ...
  return (
    <section class="nv-page nv-settings-page settings-page" aria-label="设置">
      ...
      <div class="nv-settings-layout">
        ...
        <aside class="nv-settings-preview" aria-label="主题实时预览"><div class="nv-settings-preview__screen"><div class="nv-preview-top"><span>{activeTheme().englishName}</span><strong>FOCUS</strong></div><div class="nv-preview-route"><span /><i /><b /></div><div class="nv-preview-card"><small>当前一段</small><strong>{activeTheme().name}主题</strong><span>预览实时更新</span></div><div class="nv-preview-orb" /></div><div class="nv-settings-preview__meta"><span>LIVE PREVIEW</span><strong>{activeTheme().name}</strong><small>{activeTheme().description}</small></div><button type="button" class="primary-button nv-settings-save" onClick={props.onSaveVisualSettings}><Save size={16} strokeWidth={1.8} aria-hidden="true" />保存设置</button><p class="nv-settings-saved"><span />预览实时更新 · 已保存</p></aside>
      </div>
    </section>
  );
}
