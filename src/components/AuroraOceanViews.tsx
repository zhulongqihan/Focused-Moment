        ...
        <div class="ao-orbit-stage" aria-label="今日潮汐轨道">
          ...
          <For each={sequence()}>
            {(item, index) => {
              ...
              return (
                <button type="button" classList={{ "ao-orbit-node": true, "is-complete": completed, "is-current": active }} style={"left:" + String(point.left) + "%;top:" + String(point.top) + "%;"} aria-label={`${item.title}，${item.scheduledTime || (completed ? "已完成" : "待安排")}`} title={item.title} disabled={props.busy() || completed || props.timerHasProgress()} onClick={() => props.onUseTodo(item)}>
