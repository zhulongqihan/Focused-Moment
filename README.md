...
当前稳定版本是 [v2.10.9](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.9)。Night Valley 计时页已验证开始后主窗口隐藏、专注悬浮窗显示、返回后入口重新出现在计时卡片标题栏，并保持同一计时状态；暂停、继续、完成保存、窗口尺寸和重启读取也已完成验证。全屏说明限定为无边框窗口的全屏几何与布局适配，Night Valley 当前没有独立的 F11 切换命令。
