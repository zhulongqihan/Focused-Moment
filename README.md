...
当前稳定版本是 [v2.10.13](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.13)（Windows-only，2026-09-13 发布）。本版本为 Windows 通知区域托盘加入动态计时状态、暂停/继续、专注悬浮窗和页面快速跳转；v2.10.12 修复的 Night Valley 计时页“查看专注记录”按钮文字居中问题仍包含在当前版本中。v2.10.11 接入的本地“每日一句”仍保持同一天跨主题同句，并分别以夜谷注脚、纸页页边手记、控制台讯息条、潮汐寄语和书签纸条呈现。全屏说明限定为无边框窗口的全屏几何与布局适配，当前没有独立的 F11 切换命令。
