...
当前稳定版本是 [v2.10.17](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.17)（Windows-only，2026-09-14 发布）。本版本重构 Night Valley“待办”页：以今日路径看板承载待开始、进行中和已完成三种状态，把下一步行动放到右侧工作台，并让长完成列表在面板内滚动、窄屏按顺序堆叠；同时保留 v2.10.16 的无边框窗口右上角三个控件修复、v2.10.15 的记录页点线对齐与专注时段统计、v2.10.14 的首屏自然流布局和长历史可视性，以及更早版本的托盘、计时入口和每日一句能力。全屏说明限定为无边框窗口的全屏几何与布局适配，当前没有独立的 F11 切换命令。
