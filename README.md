...
当前稳定版本是 [v2.10.16](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.16)（Windows-only，2026-09-13 发布）。本版本修复无边框窗口右上角三个控件在记录页点击时可能被顶栏拖拽事件吞掉的问题，并保留 v2.10.15 的 Night Valley“记录”页点线对齐、按专注时段统计分布和全部记录回看能力；同时保留 v2.10.14 的首屏自然流布局、主题层级和长历史可视性，以及 v2.10.13 的 Windows 通知区域托盘快速操作和 v2.10.12 的计时页“查看专注记录”按钮文字居中修复。v2.10.11 接入的本地“每日一句”仍保持同一天跨主题同句，并分别以夜谷注脚、纸页页边手记、控制台讯息条、潮汐寄语和书签纸条呈现。全屏说明限定为无边框窗口的全屏几何与布局适配，当前没有独立的 F11 切换命令。
