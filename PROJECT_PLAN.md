...
| 字段 | 当前值 |
| --- | --- |
| as_of / updated_at | 2026-09-11；根据最新截图反馈，Night Valley 第一套主题的“计时”页已定位并修复“今日记录数”与计时运行态不一致问题，v2.10.7 正在执行完整验证与发布闭环 |
| 产品目标 | 本地优先的桌面专注工具：待办 → 专注 → 桌面提醒 → 保存记录 → 回看投入 → 可恢复地长期保留 |
| 当前阶段 | 五套主题均已接入；v2.10.6 是线上基线；当前只处理第一套主题 Night Valley 的“计时”页，REFINE-11 正在修复并发布数据一致性问题，不自动展开其他主题或页面 |
| 代码基线 | v2.10.6 发布提交为 `0ec12d8`（`0ec12d810a88516e6db8bbe4b587d100af82627f`），tag `v2.10.6` 固定在同一提交；v2.10.5 tag 固定在 `e7df3e6`；v2.10.4 tag 固定在 `7662911`；v2.10.3 tag 固定在 `c4176ee`；v2.10.2 tag 固定在 `fef30c7`；v2.10.1 tag 固定在 `4ceabf9`；v2.10.0 tag 固定在 `ce15ff0` |
| 当前工作分支 | `main`；旧 CORE-01/CORE-02/CORE-03/CORE-04/CORE-05/NV-01/NV-02/NV-03/NV-04/NV-05/NV-06/DESK-01/DESK-02/DESK-03/ARCH-01/QA-01/PERF-01/TH-02/TH-03/TH-04/TH-05/DATA-01/REL-01 已完成；REFINE-03/REFINE-04/REFINE-05 DONE；REL-02/REL-03/REL-04 DONE；REFINE-06 REVIEW；REL-05 DONE；REFINE-07 REVIEW；REL-06 DONE；REFINE-08 REVIEW；REL-07 DONE；REFINE-09 REVIEW；REL-08 DONE；REFINE-10 REVIEW；REL-09 DONE；REFINE-11 DOING；REL-10 TODO |
| 发布基线 | GitHub `v2.10.6`，tag peeled commit 为 `0ec12d8`；Checks `34558337330`、macOS Native Smoke `34558337332`、macOS Universal Release `34558410140` 均 PASS；Release 四项资产均为 uploaded，digest 见 `docs/v2.10.6/RELEASE_NOTES.md`；旧 `v2.10.5`/`v2.10.4` tag 与 Release 固定且未移动 |
| 远程 main | `origin/main` 已更新至 `84cf82324ba3cb3d577e0e2308ca5914a91278ad`（包含 v2.10.6 发布代码与最终证据）；`v2.10.6` tag 仍固定在 `0ec12d8`，GitHub Release 已收口；旧 tag 未移动 |
| 本轮交付 | 已确认 Night Valley 计时页把 `TimerSnapshot.completedFocusCount`（当前计时引擎轮次）误当作当天已保存记录数；已改为读取与今日页相同的 `AnalyticsSnapshot.todaySessionCount`，补充“今日有记录但运行态为 0”的回归；v2.10.7 完整发布门槛进行中 |
| 下一项 | 完成本地全量验证、正式打包、提交推送、远程 CI、GitHub Release 与四项资产 digest 核对；仍只处理 Night Valley 计时页 |
| 当前执行人 / 在做任务 | Codex / REFINE-11 数据一致性修复与 REL-10 发布闭环进行中 |
...
| pending | REFINE-11 / REL-10 发布闭环进行中，之后等待用户验收；REFINE-10/REFINE-09/REFINE-08/REFINE-07 历史页面仍按既有验收状态保留；资源出处补录缺口仍按既有清单保留为后续治理事项 |
...
| REFINE-11 | P1 | DOING | REL-09 | Night Valley 计时页第七轮数据一致性修复：将“今天已记录/今日已完成”统一接入按日期汇总的 `todaySessionCount`，覆盖已有记录但计时运行态为 0 的场景；v2.10.7 正在验证 |
| REL-10 | P1 | TODO | REFINE-11 | v2.10.7 Night Valley Timer 今日记录数修复发布闭环：版本同步、完整验证、Windows/macOS 资产、远程 main/CI、GitHub Release 与 digest 核对 |
...
| 2026-09-11 | REFINE-11 / REL-10 | `aba70db`→工作区（未提交）；修改 `src/components/NightValleyViews.tsx`、`src/MainShell.tsx`、`tests/app.spec.mjs`，同步 v2.10.7 版本源、README、计划与发布说明；计时页改用与今日页一致的 `AnalyticsSnapshot.todaySessionCount`，新增当天已有记录而 `TimerSnapshot.completedFocusCount` 为 0 的回归；`pnpm check`、定向 Playwright 1/1 PASS，完整前端/Rust/构建/打包/远程发布待执行 | REFINE-11 DOING；下一步完成 v2.10.7 发布闭环 |
