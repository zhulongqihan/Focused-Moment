...
| 下一项 | 无待收口项：`REL-23` 已完成；下一任务按项目优先级排队，macOS 继续冻结 |
| 当前执行人 / 在做任务 | Codex / `REL-23` v2.11.0 版本纠正与 Windows-only 发布闭环已完成 |
...
| pending | 本轮无技术阻塞；远程 Checks 已成功，唯一记录为既有“每日一句”跨主题测试出现 1 flaky，workflow 总体 success；macOS 版本更新冻结，恢复前需用户明确授权；GitHub Actions 有 Node.js 20 弃用提示，但本轮检查成功且提示来自现有 action 版本；Night Valley 当前没有独立 F11/全屏产品命令，本轮结论限定为无边框窗口的全屏几何和 CSS 全屏布局；资源出处补录缺口仍按既有清单保留 |
...
- REL-12 与 REFINE-13 均收口为 DONE；最终证据文档已随 `9e7091bc33cbdaef40aeddade86123b57683f658` 正常推送，本目标完成。v2.10.8 Release/tag/资产继续只读保护。

### 2026-09-12 本地旧归档与生成物清理（已完成）

- 用户明确要求删除 v2.9.0 之前的本地归档及相关内容，并清理此前发现的 80+ GiB 构建产物；本次范围限定为本机文件，不扩展到 Git 历史、GitHub Release 或远程 tag。
- 删除前盘点确认：外部维护归档 `F:\Focused Moment Maintenance Archive 20260911-2016` 约 84.00 GiB，当前 `src-tauri\target` 约 6.33 GiB，临时 clean checkout 约 4.76 GiB；另有可再生依赖、dist、旧测试证据和 ignored 旧版本副本。
- 精确目标、字节数、引用核对和保护边界已写入 `docs/maintenance/destructive-cleanup-manifest-20260912.md`；执行前未发现正在使用 Focused Moment/Cargo/Playwright/项目 pnpm 命令的进程。
- 明确保留 `F:\Focused Moment\Focused Moment Backups`、v2.9.0 及之后的证据、当前 v2.10.9 暂存资产、发布 manifest、主题素材、现有 QA 证据、Git 已跟踪历史文档、`.git` 和远程对象；删除完成后回填结果并重新核对工作树。
- 已删除清单内全部 12 个目录、9 个显式文件和 95 个 ignored 旧版本文件；删除目标残留 0、保护目标缺失 0，备份仍为 2 个文件。清理结果已写入清单，应用源码未改动，后续本地构建需按 lockfile 重建生成物。

### 2026-09-13 REL-14 五套主题“今日一句”发布闭环（已完成）

- 用户反馈上一张截图中看不到每日一句；复核确认截图运行的是尚未包含本轮改动的 v2.10.10 旧进程。当前源码已在五套主题各自的 Today 页面接入同一份按日期稳定选择的本地语料：Night Valley 放在标题说明下方、路径上方；Editorial Paper 放在页眉页边；Graphite Console 放在标题与序列之间；Aurora Ocean 放在主行动按钮前；Botanical Library 放在开始行动前的书签纸条位置。
- 新增共享 `DailyFocusLine` 组件与主题化 CSS，保留现有出处、英文原文/中文译文逻辑；同一日期切换主题时句子 ID 保持一致，只改变呈现材质与位置。Playwright 回归新增五主题同日稳定性断言。
- 本地证据：`pnpm check` PASS；前端串行回归 `68/68` PASS；`pnpm build` PASS（2066 modules）；Rust fmt/check/test PASS（33/33）；Windows portable、Setup、MSI 均生成且文件版本为 2.10.11；五主题 Today 截图和压力宽度专项通过。
...
### 2026-09-13 REL-15 Night Valley 计时页“查看专注记录”对齐（已完成，Windows-only 发布）

- 用户反馈截图中的“查看专注记录”按钮文字没有处于按钮正中。复核确认根因是箭头与文字参与同一段普通横向 flex 排版，按钮整体内容以“文字+箭头”组合居中，文字因此被推向左侧；原有布局覆盖还主要位于桌面断点，窄窗口更容易暴露偏移。
- 最小改动仅涉及 `src/components/NightValleyViews.tsx`、`src/App.css` 和 `tests/today-visual.spec.mjs`：将按钮文字包成独立 `span` 并让箭头绝对定位在右侧，公共规则覆盖 2560、1024、540 三类计时视口；不改变按钮尺寸意图、点击动作、记录页、其他主题或其他页面。
- 本地证据：`pnpm check` PASS；Night Valley timer 定向回归 `4/4` PASS（含按钮几何居中、全屏布局、1 小时读数和缩放全屏）；`pnpm build` PASS（2066 modules）；Cargo fmt/check/test locked PASS（33/33）；`pnpm package:release` PASS，Windows portable、Setup、MSI 均为 v2.10.12；`git diff --check` PASS。完整前端 69 项本轮未执行，因本次改动已由目标范围定向覆盖，发布说明明确记录。
...
- **第一批已实现**：
  1. 动态只读状态：未开始、专注中/已暂停、已用时或剩余时长、当前事项标题；
  2. 根据状态提供“暂停计时”或“继续计时”；
  3. “打开专注悬浮窗”，复用已有窗口，不重复创建；
  4. “打开计时页”；
  5. “打开今日”“打开待办”“打开记录”三个导航入口；
...
### 2026-09-13 REFINE-14 / REL-17 Night Valley“记录”页三项问题（已完成，Windows-only 发布）

- **根因**：记录页仍被旧的 measured layout 规则钉在固定画布上，并将源码中的归档摘要压缩为 1×1、透明且不可交互；统计区、详情区和分布区也依赖固定高度/绝对定位，因此窗口高度变化时会出现首屏折叠和信息被截断。全部历史记录则直接在页面主流中展开，天数增加后把后续内容推得过长。
- **实现**：仅针对当前截图对应的 Night Valley“记录”页恢复自然文档流；摘要、七日时间轴、统计条、选中日期详情、分布和趋势按主题语义重新分层，保留夜谷的雾面深色、金色路径与薄边界；详情卡在桌面端采用主信息 + 仪表双列，在窄窗口自动堆叠；全部记录保留所有日期，但收纳进带键盘友好焦点、滚动条和 overscroll 隔离的固定高度列表，避免长历史撑爆页面。
- **代码与测试**：修改 `src/App.css`、`src/components/NightValleyViews.tsx`、`tests/app.spec.mjs`、`tests/today-visual.spec.mjs`；新增 28 天历史夹具、长列表内部滚动/末日展开回归和归档主区非零可见性断言。`pnpm check`、`git diff --check`、完整前端串行 `71/71` PASS；Night Valley 记录页定向、5 项页面几何/压力宽度专项、Vite build、Rust `fmt/check/test --locked`（34/34）及 Windows v2.10.14 三项打包均 PASS。
- **发布结果**：按用户要求将本界面问题集中修完后一次性进入 `REL-17`；发布代码提交 `fb1e03eb248e8415e2e29bc688cd756458b427b5`，`v2.10.14` tag 已推送，Windows Checks `34756725964` 通过，GitHub Release 已正式发布且三项 Windows 资产 digest 与本地 SHA-256 一致。不构建/上传 macOS 资产，不运行 macOS Release 或 Native Smoke。

### 2026-09-13 REFINE-15 / REL-18 Night Valley“记录”页可读性修复（已完成，Windows-only 发布）

- **用户问题**：七日总览的点和线没有完全对上；分布区表述模糊且重复表达按日趋势；全部记录出现空条，无法正常回看。
- **根因**：七日图的 SVG 只占地图中间区域，而日期节点按地图完整尺寸定位，且路径首段未经过首个节点；分布区复用了按日总投入时长；历史分组还受旧可见性规则和默认折叠状态影响。
- **实现**：让 SVG 填满记录地图并让路径经过首尾节点；分布区改为按记录完成时间聚合凌晨、上午、下午和晚间的专注时长与段数；日期分组默认展开，保留可收起和列表内滚动回看能力。修改集中在 `src/App.css`、`src/MainShell.tsx`、`src/components/NightValleyViews.tsx` 和 `tests/app.spec.mjs`。
- **本地验证**：`pnpm check`、完整前端 `72/72`、Vite build（2066 modules）、Rust `fmt/check/test --locked`（34/34）、`git diff --check` 和 `pnpm package:release` 均通过；Windows 三项 v2.10.15 资产 SHA-256 见 `docs/v2.10.15/RELEASE_NOTES.md` 与 `docs/qa/REFINE-15-night-valley-records-v2.10.15.md`。
- **发布结果**：发布代码提交 `88d97ad120024e63407e2702bee7359ba68cfa25` 已推送到 `main`，`v2.10.15` tag 已推送且 peeled commit 一致；Windows Checks [`34763893913`](https://github.com/zhulongqihan/Focused-Moment/actions/runs/34763893913) 成功，远程前端步骤记录 71 passed + 1 flaky（既有“每日一句”跨主题测试的 Chromium 资源波动，workflow 总体 success），Rust fmt/check/test 全部通过；GitHub Release [`v2.10.15`](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.15) 三项 Windows 资产均 uploaded，远端 digest 与本地 SHA-256 一致。不构建/上传/运行 macOS 版本链路。
...
2026-09-09 23:22：用户指出启动协议遗漏项目最高优先级的版本更迭、版本源同步、正式打包、提交、推送、远程 CI、GitHub Release 资产与 digest 核对。本工作单元已将这些步骤改为强制完成条件，并明确仅文档/计划变更的例外；本次只修正流程文档，不升应用版本、不重打 `v2.10.0` 资产。

**进度读取规则：**最新明确用户要求 > 当前源码/运行结果/远程事实 > 本看板 > 历史文档。后续开始工作先刷新以上版本和提交，不能把今天的基线当永久事实。任务勾选必须有结果证据，不能按版本号、文件数或截图数量计算“完成百分比”。

### 2026-09-11 最新用户纠正：计时修复暂停，先做项目瘦身

用户最新实测取代了旧的“REFINE-12 已可验收”判断：点击开始后专注悬浮窗会弹出，但主界面没有自动隐藏；返回主界面后，运行中的计时页仍找不到再次打开悬浮窗的入口。本条是当前有效事实，不能被此前的 Playwright/Tauri mock 通过结果覆盖。

本工作单元只做状态对账、只读扫描和计划落盘，不修改计时/窗口代码，不删除任何文件，不重写 README，不删除远程分支、tag、Release 资产或 Actions 产物。完整范围、证据、清理顺序、回滚和验收门槛见 `docs/maintenance/PROJECT_SLIMMING_PLAN.md`。

用户提到的“为什么一个小 bug 会花很久”暂按三个可验证因素记录：每次用户可见代码变更都要经过版本同步、测试、打包、提交、推送、远程 CI、跨平台资产和 Release 闭环；当前 `src/App.css` 与 `src/MainShell.tsx` 仍是大体量共享壳层，局部级联/状态依赖会放大改动；浏览器 mock 通过与 Windows 原生窗口行为之间存在证据落差。上述是维护假设，后续以 MAINT-01 的依赖/引用审计和 REFINE-13 的原生复现验证，不预先把任何一项写成唯一根因。
...
| REFINE-06 | P1 | REVIEW | REL-03 | Night Valley 今日页截图驱动局部精修：第一轮已完成节点信息不丢失、路径节奏收紧、Today 快速命令入口移除、右卡改为下一节点信息并跳转计时页；第二轮已补齐全屏底部总结可见与至少四个 >90° 路径回折，v2.10.2 已发布，待用户验收 |
...
| REL-05 | P1 | DONE | REFINE-06 | v2.10.2 Night Valley Today 第二轮精修发布：版本同步、完整本地门槛、Windows 包、远程 main/CI、macOS Universal 资产、GitHub Release 与 digest 证据收口 |
| REFINE-07 | P1 | REVIEW | REL-05 | Night Valley 今日页第三轮截图驱动精修：移除硬折线，改为多弧段连续路径；增加日期后的实时钟；右卡重构为今日概览；v2.10.3 已发布，待用户验收 |
...
| REFINE-10 | P1 | REVIEW | REL-08 | Night Valley 计时页第六轮截图驱动修复：针对 Windows 高 DPI 全屏下 60 分钟读数与统计区重叠，修正卡片 flex 收缩、低高度桌面间距和内容完整性断言；v2.10.6 已发布，待用户验收 |
...
| REFINE-14 | P1 | DONE | REL-16 | Night Valley“记录”页已完成首屏折叠、主题层级与长历史可视性修复；源码、完整前端回归和 Windows 发布均已完成 |
...
| 2026-09-11 | MAINT-06 | `ae6ac45`→工作树 | 在 `F:\Focused Moment Clean Checkout 20260911-2028` 以 detached `ae6ac45` 重装依赖，完成 `pnpm check`、`pnpm build`、前端 67/67、Cargo fmt/check/test（33/33）、`pnpm tauri build --debug`；以隔离 `LOCALAPPDATA` 启动真实 `focused-moment.exe`，获得窗口句柄/标题并关闭；刷新后 worktree clean；工作区备份仍 2 个文件 | 证据见 `docs/maintenance/cleanup-manifest-20260911-2016.md`；干净构建输出未包含两个已删除 SVG；启动只证明窗口可创建/关闭，不推断计时浮窗生命周期已修复 | 版本/Release 不变；v2.10.8 未移动/删除；维护提交 `ae6ac45` 待推送/CI；维护阶段解除 REFINE-13 | REFINE-13（真实 Windows 原生复现） |

| 2026-09-12 | UX-AUDIT-01 / REL-13 | `b7cc70f`→`46f42e2`；完成五套主题 25 页 × 3 轮 = 75 轮真实浏览器体验，另完成 50 组紧凑密度专项、20 个设置锚点、图表日期点和关键截图人工复核；修复跨 Tab 滚动复位、无效记录占位、过期设置说明、Editorial 移动导航换行、Aurora/Botanical 图表按钮 0×0；同步测试契约和 v2.10.10 版本源/README/发布说明；本地 `pnpm check`、`pnpm build`、前端 67/67、Rust 33/33、Windows release bundle 与 SHA-256 均通过；已提交并推送 main、创建 tag/Release，Checks `34636727552`、macOS Native Smoke `34636727575`、macOS Release `34636745046` 均 PASS，Release 四项资产 digest 已核对 | 详细矩阵与根因见 `docs/qa/UX-AUDIT-01-all-themes-20260912.md`；UX-AUDIT-01 完成；v2.10.10 发布已收口；v2.10.8/v2.10.9 未触碰 | 用户验收 v2.10.10 |
...
### 2026-09-13 REFINE-16 / REL-19 Night Valley 记录页窗口控件回归（已完成，Windows-only 发布）
...
- **修复**：顶栏拖拽判断排除 `.window-controls`，控件容器停止 `mousedown` 冒泡；新增三控件命中、命令调用及不触发拖拽的前端回归测试。版本已同步至 v2.10.16。
...
- **发布证据**：代码提交 `e6671fceae2595cf0fa4a6893d3dbc059082d651`、tag `v2.10.16` 已推送；Windows Checks `34766432213` 成功（远程前端 73 passed、Rust fmt/check/test 通过）；GitHub Release [v2.10.16](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.16) 三项资产均 uploaded，远端 digest 与本地 SHA-256 一致。
- **当前状态**：REFINE-16 DONE；REL-19 DONE；macOS 继续冻结；旧 v2.10.14 用户进程未被终止。


### 2026-09-14 REFINE-17 / REL-20 Night Valley 待办工作台重构（已完成，Windows-only 发布）

- **用户反馈**：待办页中间文字仍有重叠，界面留白较多，已完成记录无法正常查看；此前记录页右上角三个原生窗口控件不可用，要求保持可检查。
- **根因与修复**：旧待办页使用固定绝对坐标，内容长度和窗口尺寸变化时缺少共同流布局；已完成行的隐式三列网格承载五个元素，导致恢复/删除错位。新增今日路径与完成进度，重建待办页文档流网格和右侧下一步工作台；完成列表改为面板内滚动，完成行显式五列，窄屏按序堆叠。
- **修改文件**：`src/components/NightValleyViews.tsx`、`src/App.css`、`tests/app.spec.mjs`；同步 `package.json`、Cargo/lock、`tauri.conf.json`、`runtime.rs`、README、`docs/v2.10.17/RELEASE_NOTES.md` 和 `docs/qa/REFINE-17-night-valley-todo-v2.10.17.md`。
- **本地证据**：`pnpm check` PASS；完整前端 75/75 PASS；Night Valley 尺寸/安全可达性专项 3/3 PASS；待办/逾期/窗口控件专项 9/9 PASS；`pnpm build`（2066 modules）PASS；Rust fmt/check/test（34/34）PASS；`pnpm package:release` PASS。三项 Windows 资产 SHA-256 已记录，见 `docs/v2.10.17/RELEASE_NOTES.md`。
- **发布证据**：代码提交 `67104979f79591125cda32f5506836911b0279e1`、tag `v2.10.17` 已推送；Windows Checks `34770039344` 成功（远程前端 75 passed、Rust fmt/check/test 通过）；GitHub Release [v2.10.17](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.17) 三项资产均 uploaded，远端 digest 与本地 SHA-256 一致。
- **当前状态**：REFINE-17 DONE；REL-20 DONE；macOS 继续冻结。


### 2026-09-14 REFINE-18 / REL-21 Night Valley 设置工作台重构（已完成，Windows-only 发布）

- **用户反馈**：设置页存在文字重叠；视觉强调、动效强度和紧凑/舒展变化价值低；音效选项太少；快捷键区无实际用途；右侧 Night Valley 预览图观感差且用途不清；“保存设置”语义不符合修改后自动保留的预期。
- **根因与修复**：旧设置页由多组固定绝对坐标、固定高度和旧媒体查询共同控制，新增文案与控件没有共享内容流，导致覆盖和裁切。现在改为自然流式三栏/断点布局，行为说明与开关各自占据独立行；删除视觉/动效滑杆、密度和快捷键；增加七种内置提示声并同步 Rust 枚举；右侧改为展示当前主题、实时状态和自动保存反馈的“主题观测站”；主题、提醒开关和音效选择均在修改后自动保存，并对失败给出明确提示。
- **修改范围**：`src/components/NightValleyViews.tsx`、`src/App.css`、`src/MainShell.tsx`、`src/lib/contracts.ts`、四套其它主题设置组件、`src-tauri/src/runtime.rs`、`tests/app.spec.mjs`、`tests/today-visual.spec.mjs`；同步版本源、README、发布说明和 QA 报告。
- **本地证据**：`pnpm check`；`tests/app.spec.mjs` 40/40；`tests/today-visual.spec.mjs` 35/35；`pnpm build`（2066 modules）；Cargo fmt/check/test locked（35/35）；`pnpm package:release`；三项 Windows 产物版本元数据和 SHA-256 已核对，见 `docs/v2.10.18/RELEASE_NOTES.md`。完整套件首次因 Vite 服务器提前退出造成连接拒绝，随后按文件串行复跑并全部通过。
- **发布证据**：代码提交 `6e8a076e9864a19a8e390e432ff8384aef0e5462`、tag `v2.10.18` 已推送；Windows Checks [`34774739411`](https://github.com/zhulongqihan/Focused-Moment/actions/runs/34774739411) PASS；GitHub Release [v2.10.18](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.18) 三项资产均 `uploaded`，远端 digest 与本地 SHA-256 一致。
- **当前状态**：REFINE-18 DONE；REL-21 DONE；Windows-only 发布已收口；macOS workflow 未执行并继续冻结；未停止用户进程。

### 2026-09-14 REFINE-19 计划建立（待执行）
...
### 2026-09-14 REFINE-19 / REL-22 Windows-only 发布闭环（已完成）

- **逐条结果**：严格按 `TODAY-01～11 → TIMER-01～21 → TODO-01～03 → RECORDS-01～07 → SETTINGS-01～06` 执行，共 48 个编号；每条均已填写第一主题原话、编辑纸页对应区域、复现结果、根因、实际修改/未修改范围、定向测试、截图证据和最终状态。
- **本地验证**：升版前后 REFINE-19 定向回归均为 `48/48 PASS`；`pnpm check`、`pnpm build`（2066 modules）、`git diff --check`、Cargo fmt/check/test（35/35）和 `pnpm package:release` 均 PASS。
- **提交与 tag**：产品提交 `8ef76813f65f86750fab46d2dba5f158b2aee0f7`；测试契约后续提交 `2afbc04d46f5c46cf81992e0bf186b30d43a9ba2`；`v2.10.19` tag 固定指向产品提交，未移动。
- **远程证据**：Windows Checks [`34835489340`](https://github.com/zhulongqihan/Focused-Moment/actions/runs/34835489340) PASS，报告 `123 passed`；GitHub Release [`v2.10.19`](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.10.19) 三项 Windows x64 资产均已上传，远端 digest 与本地 SHA-256 一致。
- **保护边界**：未运行或修改任何 macOS workflow；未停止用户进程；PID `23304` 及 WebView2 子进程保持运行，旧 v2.10.18 文件按占用警告保留。
- **最终状态**：`REFINE-19 DONE`、`REL-22 DONE`；下一任务按项目优先级排队。

### 2026-09-14 19:15 用户版本纠正（REL-23，已完成）

- **最新事件**：用户明确纠正“本轮切换了一个界面，版本应为 `2.11.0`”。该事件优先于此前将本轮标为 `v2.10.19` 的派生结论。
- **冲突处理**：`v2.10.19` 已发布这一事实保留为历史记录；不移动 tag、不覆盖 Release、不伪装为 `v2.11.0`，当前源码和新发布目标切换到 `2.11.0`。
- **当前进度**：所有版本源已同步为 `2.11.0`；`pnpm check`、`pnpm build`、48/48 Editorial Paper 回归、Cargo fmt/check/test 和 `pnpm package:release` 均已通过，资产 SHA-256 已回填新 Release notes。
- **发布结果**：提交 `600501a` 已推送，`v2.11.0` tag 已创建且固定，Windows Checks `34838375564` 已 PASS，GitHub Release 与 portable、Setup/NSIS、MSI 三项资产均已完成远端 digest 核对；macOS workflow 继续不运行，当前运行中的用户进程 PID `12200` 未被停止。

### 2026-09-14 南大就业宣讲会待办数据录入（已完成）

- **用户事件**：用户要求获取公众号“南大就业”中 2026-09-15 起的宣讲会，并把时间、地点加入待办；本次明确不改样式。
- **数据范围**：按“9 月 15 日起”解释为 2026-09-15（含当天），录入当天 15 条正式宣讲/招聘场次；排除同日两条网易游戏雷火路演记录。官方接口在 9/15～9/30 查询到 150 条活动，后续日期未批量写入，以避免一次性污染待办列表。
- **来源**：南京大学就业门户公开专场活动接口 `https://job.nju.edu.cn/api/career/job/fair/specifics`；活动标题、时间段和校区/场地已写进待办标题，开始时间写入 `scheduledTime`。
...
### 2026-09-16 REL-30 v2.11.7 Night Valley 记录、待办与通知修复（已完成）
...
- **发布闭环**：产品提交 [`6e7cefb`](https://github.com/zhulongqihan/Focused-Moment/commit/6e7cefb8b43e044e4f3a7efd231a3b6250addb01) 已推送到 `main`；`v2.11.7` tag 已创建并固定指向该提交；GitHub Release [`v2.11.7`](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.11.7) 已发布，三项 Windows x64 资产均为 `uploaded`，远端 digest 与本地 SHA-256 一致。
...
### 2026-09-16 REL-31 v2.11.8 跨主题壳层、通知与极光海面布局修复（已完成）
...
- **候选资产**：Windows x64 portable、Setup/NSIS、MSI 已生成；portable / Setup / MSI 分别为 `23,852,032` / `16,194,496` / `17,186,816` bytes，SHA-256 已写入 `docs/v2.11.8/RELEASE_NOTES.md`；旧 v2.11.6 文件因仍被占用未删除，不影响新资产。
- **发布闭环**：产品提交 [`a397057`](https://github.com/zhulongqihan/Focused-Moment/commit/a397057df2cc3950e984f88029869cd16cb65379) 已推送到 `main`；`v2.11.8` tag 已创建并固定指向该提交；GitHub Release [`v2.11.8`](https://github.com/zhulongqihan/Focused-Moment/releases/tag/v2.11.8) 已发布，三项 Windows x64 资产均为 `uploaded`，远端 digest 与本地 SHA-256 一致。
